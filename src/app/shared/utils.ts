...
export const pdfSetup = (period, logs) => ({
  header: [{
    margin: [ 0, 10, 0, 20 ],
    height: 30,
    alignment: 'center',
  ...
  footer: (currentPage, pageCount, pageSize) => ({
    margin:  [ 40, 0, 40, 0 ],
    columns: [
        { text: 'Powered by Hexigin', color: '#565656' },
        { text: `${currentPage} of ${pageCount}`, alignment: 'right' }
    ]
  }),
  content: [
    { text: 'CleonTime Logs', style: 'header' },
    { text: `${period}`, style: 'sub-header' },
    {
      table: {
        widths: ['25%','15%','25%', '35%'],
        body: [
          [{text: 'Timestamp', style: 'table-header'}, {text: 'User ID', style: 'table-header'}, {text: 'Name', style: 'table-header'}, {text: 'Activity', style: 'table-header'}],
          ...logs
        ]
      }
    }
  ],
  watermark: { text: 'Cleontime', opacity: 0.1 },
  styles: {
      header: {
          fontSize: 18,
    bold: true,
